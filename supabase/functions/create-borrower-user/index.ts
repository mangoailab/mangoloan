...
function appError(message: string, details?: Record<string, unknown>) {
  return json({ ok: false, error: message, ...(details || {}) });
}
...
Deno.serve(async (req) => {
  ...
  if (aal !== "aal2") return appError("Please complete admin 2FA before managing portal users.");
  ...
  if (action === "reset-password") {
    if (!borrower.auth_user_id) return appError("Borrower does not have a linked portal user.");
    ...
    if (updateUserError || !updated?.user) {
      return appError(updateUserError?.message || "Could not reset temporary password.");
    }
    return json({
